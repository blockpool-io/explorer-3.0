<template>
  <loader :data="transactions">
    <table-component v-if="transactions && transactions.length > 0" :data="transactions" sort-by="timestamp" sort-order="desc" :show-filter="false" :show-caption="false" table-class="w-full">
      ...
      <table-column show="amount" :label="$t('Amount (token)', { token: networkTokenShortName() })" header-class="right-header-end-cell lg:pr-4" cell-class="right-end-cell lg:pr-4">
        <template slot-scope="row">
      ...
      <table-column show="fee" :label="$t('Fee (token)', { token: networkTokenShortName() })" header-class="right-header-end-cell hidden lg:table-cell" cell-class="right-end-cell hidden lg:table-cell">
        <template slot-scope="row">
