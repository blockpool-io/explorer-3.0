...
class CryptoCompareService {
  async price(currency) {
    const response = await axios.get(`https://min-api.cryptocompare.com/data/price?fsym=BPL&tsyms=${currency}`)
  ...
  async sendRequest(type, limit, dateTimeFormat) {
    ...
    if (store.getters['currency/name'] !== store.getters['network/tokenShortName']) {
    ...
    const response = await axios
      .get(`https://min-api.cryptocompare.com/data/histo${type}`, {
        params: {
          fsym: store.getters['network/tokenShortName'],
          tsym: targetCurrency,
          toTs: date,
          limit
        }
